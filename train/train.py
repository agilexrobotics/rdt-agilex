...
def print_memory_usage():
    ...
    process_rss = mem_info.rss / (1024 ** 2)  # 转换为 MB
    process_vms = mem_info.vms / (1024 ** 2)  # 转换为 MB

    # 获取系统内存使用情况
    sys_mem = psutil.virtual_memory()
    sys_total = sys_mem.total / (1024 ** 2)  # 转换为 MB
    sys_available = sys_mem.available / (1024 ** 2)  # 转换为 MB
    sys_used = sys_mem.used / (1024 ** 2)  # 转换为 MB
    sys_percent = sys_mem.percent

    # 获取系统交换分区使用情况
    sys_swap = psutil.swap_memory()
    swap_total = sys_swap.total / (1024 ** 2)  # 转换为 MB
    swap_used = sys_swap.used / (1024 ** 2)  # 转换为 MB
    swap_percent = sys_swap.percent

    # 打印详细信息
    print(f"Current process memory usage (RSS): {process_rss:.2f} MB")
    print(f"Current process memory usage (VMS): {process_vms:.2f} MB")
    print(f"System total memory: {sys_total:.2f} MB")
    print(f"System available memory: {sys_available:.2f} MB")
    print(f"System used memory: {sys_used:.2f} MB")
    print(f"System memory usage: {sys_percent}%")
    print(f"System swap total: {swap_total:.2f} MB")
    print(f"System swap used: {swap_used:.2f} MB")
    print(f"System swap usage: {swap_percent}%")
...
def train(args, logger):
    ...
    # Get preprocessing statistics from dataset for later saving
    preprocessing_stats = None
    if args.load_from_hdf5:
        try:
            # If using HDF5 dataset, try to get statistics from the dataset
            # This assumes the HDF5 dataset has computed and stored statistics
            from data.hdf5_vla_dataset import HDF5VLADataset
            hdf5_dataset = HDF5VLADataset()
            if hasattr(hdf5_dataset, 'state_mean'):
                preprocessing_stats = {
                    'state_preprocessing': hdf5_dataset.state_preprocessing,
                    'action_preprocessing': hdf5_dataset.action_preprocessing,
                    'state_mean': hdf5_dataset.state_mean,
                    'state_std': hdf5_dataset.state_std,
                    'state_min': hdf5_dataset.state_min,
                    'state_max': hdf5_dataset.state_max,
                    'state_range': hdf5_dataset.state_range,
                    'state_norm_mean': getattr(hdf5_dataset, 'state_norm_mean', None),
                    'state_norm_std': getattr(hdf5_dataset, 'state_norm_std', None),
                    'action_mean': hdf5_dataset.action_mean,
                    'action_std': hdf5_dataset.action_std,
                    'action_min': hdf5_dataset.action_min,
                    'action_max': hdf5_dataset.action_max,
                    'action_range': hdf5_dataset.action_range,
                    'action_norm_mean': getattr(hdf5_dataset, 'action_norm_mean', None),
                    'action_norm_std': getattr(hdf5_dataset, 'action_norm_std', None),
                    'dataset_name': hdf5_dataset.DATASET_NAME,
                    'num_episodes': len(hdf5_dataset.file_paths)
                }
                logger.info("Obtained preprocessing statistics from HDF5 dataset, will save them together with checkpoint.")
        except Exception as e:
            logger.warning(f"Failed to obtain HDF5 dataset statistics: {e}")
            preprocessing_stats = None
        ...
        for batch in train_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_period == 0:
                    ...
                    # Save preprocessing statistics to checkpoint directory
                    if preprocessing_stats is not None:
                        import pickle
                        stats_save_path = os.path.join(save_path, "preprocessing_stats.pkl")
    ...
    if accelerator.is_main_process:
        ...
        # Save preprocessing statistics to final model directory
        if preprocessing_stats is not None:
            import pickle
            stats_save_path = os.path.join(args.output_dir, "preprocessing_stats.pkl")
            with open(stats_save_path, 'wb') as f:
                pickle.dump(preprocessing_stats, f)
            logger.info(f"Saved preprocessing statistics to final model directory: {stats_save_path}")
