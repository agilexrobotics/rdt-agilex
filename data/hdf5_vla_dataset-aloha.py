...
import glob
arm_joint_state_names = ['left', 'right']
...
class HDF5VLADataset:
    ...
    def parse_hdf5_file(self, file_path):
        ...
        with h5py.File(file_path, 'r') as f:
            qpos = np.concatenate([f[f'/arm/jointStatePosition/puppet{arm_name.capitalize()}'][()] for arm_name in arm_joint_state_names], axis=1)
            ...
            qpos = qpos / np.array(
               [[1, 1, 1, 1, 1, 1, 0.08, 1, 1, 1, 1, 1, 1, 0.08]] 
            )
            target_qpos = np.concatenate([f[f'/arm/jointStatePosition/master{arm_name.capitalize()}'][()] for arm_name in arm_joint_state_names], axis=1)
            target_qpos = target_qpos[step_id:step_id+self.CHUNK_SIZE] / np.array(
               [[1, 1, 1, 1, 1, 1, 0.08, 1, 1, 1, 1, 1, 1, 0.08]] 
            )
            ...
            cam_high = parse_img('front')
            # For step_id = first_idx - 1, the valid_len should be one
            valid_len = min(step_id - (first_idx - 1) + 1, self.IMG_HISORY_SIZE)
            cam_high_mask = np.array(
                [False] * (self.IMG_HISORY_SIZE - valid_len) + [True] * valid_len
            )
            cam_left_wrist = parse_img('left')
            cam_left_wrist_mask = cam_high_mask.copy()
            cam_right_wrist = parse_img('right')
