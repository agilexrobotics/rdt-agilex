...
class RemoteInferenceClient:
    def __init__(self, host, port, use_compression=True, socket_timeout=60.0, 
                 chunk_size=65536, socket_buffer_size=1048576, 
                 compression_level=1, max_retries=3):
        self.host = host
        self.port = port
        self.socket = None
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"Socket buffer size: {self.socket_buffer_size} bytes, compression: {self.use_compression}")
            return True
    ...
    def disconnect(self):
        ...
        if self.socket:
            self.socket.close()
            self.connected = False
            print("Disconnected from inference server")
    ...
    def send_inference_request(self, data):
        ...
        for retry in range(self.max_retries):
            if not self.connected:
                if not self.connect():
                    print(f"Retry {retry + 1}/{self.max_retries}: Failed to connect")
            ...
            try:
                ...
                request_data = pickle.dumps(data, protocol=pickle.HIGHEST_PROTOCOL)
                print(f"Serialization time: {time.time() - start_time:.3f}s")
                
                # 根据配置决定是否压缩数据
                if self.use_compression:
                    compress_start = time.time()
                    compressed_data = zlib.compress(request_data, level=self.compression_level)
                    compression_ratio = len(compressed_data) / len(request_data)
                    print(f"Compression time: {time.time() - compress_start:.3f}s, ratio: {compression_ratio:.3f}")
                    
                    # 发送压缩标志(1字节) + 原始长度(4字节) + 压缩长度(4字节) + 压缩数据
                    header = struct.pack('!BII', 1, len(request_data), len(compressed_data))
                    data_to_send = compressed_data
                    print(f"Sending compressed data: {len(compressed_data)} bytes ({len(compressed_data)/1024/1024:.2f} MB), original: {len(request_data)/1024/1024:.2f} MB")
                else:
                    # 发送未压缩标志(1字节) + 原始长度(4字节) + 数据长度(4字节) + 数据
                    header = struct.pack('!BII', 0, len(request_data), len(request_data))
                    data_to_send = request_data
                    print(f"Sending uncompressed data: {len(request_data)} bytes ({len(request_data)/1024/1024:.2f} MB)")
                ...
                while bytes_sent < data_length:
                    chunk = data_to_send[bytes_sent:bytes_sent + self.chunk_size]
                    ...
                    # 减少进度打印频率
                    if bytes_sent % (self.chunk_size * 64) == 0:  # 每64个chunk打印一次进度
                        print(f"Sent: {bytes_sent/1024/1024:.2f} MB / {data_length/1024/1024:.2f} MB")
                
                send_time = time.time() - send_start
                print(f"Data sent successfully in {send_time:.3f}s, speed: {data_length/1024/1024/send_time:.2f} MB/s")
                
                # 接收响应头部
                response_header = self._recv_exact(9)  # 1+4+4字节
                compressed_flag, original_length, compressed_length = struct.unpack('!BII', response_header)
                
                print(f"Response length: {compressed_length} bytes ({compressed_length/1024:.2f} KB)")
                if compressed_flag:
                    print(f"Response is compressed, original: {original_length} bytes")
                
                # 接收结果数据
                recv_start = time.time()
                result_data = self._recv_exact(compressed_length)
                recv_time = time.time() - recv_start
                print(f"Response received in {recv_time:.3f}s, speed: {compressed_length/1024/recv_time:.2f} KB/s")
                ...
                print(f"Socket timeout occurred on retry {retry + 1}")
                self.connected = False
                if self.socket:
                    self.socket.close()
                    self.socket = None
                ...
                print(f"Error in inference request (retry {retry + 1}): {e}")
                self.connected = False
                if self.socket:
                    self.socket.close()
                    self.socket = None
    ...
    def _recv_exact(self, size):
        ...
        while len(data) < size:
            ...
            # 减少进度打印频率
            if len(data) % (self.chunk_size * 16) == 0:  # 每16个chunk打印一次进度
...
def inference_process(args, t, ros_operator, inference_client):
    ...
    # Debug: Print data sizes before sending
    print(f"Preparing data for inference...")
    if camera_color is not None:
        print(f"Camera color shape: {camera_color.shape}, size: {camera_color.nbytes / 1024 / 1024:.2f} MB")
    if camera_depth is not None:
        print(f"Camera depth shape: {camera_depth.shape}, size: {camera_depth.nbytes / 1024 / 1024:.2f} MB")
    if camera_point_cloud is not None:
        print(f"Camera point cloud shape: {camera_point_cloud.shape}, size: {camera_point_cloud.nbytes / 1024 / 1024:.2f} MB")
    print(f"Robot state shape: {robot_state.shape}, size: {robot_state.nbytes / 1024:.2f} KB")
    if pre_instruction_attention_vector is not None:
        instruction_size = pre_instruction_attention_vector.nbytes if hasattr(pre_instruction_attention_vector, 'nbytes') else len(pre_instruction_attention_vector) * 4
        print(f"Instruction vector size: {instruction_size / 1024:.2f} KB")

    request_data = {
        'camera_color': camera_color,
        'camera_depth': camera_depth,
        'camera_point_cloud': camera_point_cloud,
        'robot_state': robot_state,
        'instruction_vector': pre_instruction_attention_vector,
        'pos_lookahead_step': args.pos_lookahead_step
    }
    
    # Estimate total data size
    import pickle
    temp_data = pickle.dumps(request_data)
    print(f"Total request data size: {len(temp_data) / 1024 / 1024:.2f} MB")
    
    result = inference_client.send_inference_request(request_data)
    
    if result is None:
        print("Failed to receive result from server")
        return
    ...
    inference_actions = action  # 1, chunk_size, action_dim
...
def model_inference(args, ros_operator):
    ...
    inference_client = RemoteInferenceClient(
        ...
        chunk_size=args.network_chunk_size,
            ...
            if args.asynchronous_inference:
                if inference_thread is None and (args.pos_lookahead_step == 0 or t % args.pos_lookahead_step == 0 or t >= max_t) and ros_operator.check_frame():
                    inference_thread = threading.Thread(target=inference_process,
                                                        args=(args, t, ros_operator, inference_client))
...
def get_arguments():
    ...
    # 远程服务器配置
    parser.add_argument('--server_host', type=str, default='localhost', help='Remote inference server host')
    parser.add_argument('--server_port', type=int, default=8888, help='Remote inference server port')
    parser.add_argument('--use_compression', action='store_true', default=True, help='Enable data compression for network transfer')
    parser.add_argument('--socket_timeout', type=float, default=60.0, help='Socket timeout in seconds')
    parser.add_argument('--network_chunk_size', type=int, default=65536, help='Data chunk size for network transfer (bytes)')
    ...
    parser.add_argument('--max_retries', type=int, default=3, help='Maximum retry attempts for network requests')
