...
class RosOperator:
    ...
    def puppet_arm_publish(self, left, right):
        ...
        joint_state_msg.name = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6', 'joint7']  # 设置关节名称
    ...
    def get_frame(self):
        ...
        while len(self.img_left_deque) > 0 :#and self.img_left_deque[0].header.stamp.to_sec() <= frame_time:
            img_left = self.img_left_deque.popleft()
        img_left = self.bridge.imgmsg_to_cv2(img_left, 'passthrough')

        while len(self.img_right_deque) > 0 :#and self.img_right_deque[0].header.stamp.to_sec() <= frame_time:
            img_right = self.img_right_deque.popleft()
        img_right = self.bridge.imgmsg_to_cv2(img_right, 'passthrough')

        while len(self.img_front_deque) > 0 :#and self.img_front_deque[0].header.stamp.to_sec() <= frame_time:
            img_front = self.img_front_deque.popleft()
        img_front = self.bridge.imgmsg_to_cv2(img_front, 'passthrough')

        while len(self.puppet_arm_left_deque) > 0 :#and self.puppet_arm_left_deque[0].header.stamp.to_sec() <= frame_time:
            puppet_arm_left = self.puppet_arm_left_deque.popleft()
        # puppet_arm_left = self.puppet_arm_left_deque.popleft()

        while len(self.puppet_arm_right_deque) > 0 :#and self.puppet_arm_right_deque[0].header.stamp.to_sec() <= frame_time:
        ...
        if self.args.use_depth_image:
            while self.img_left_depth_deque[0].header.stamp.to_sec() <= frame_time:
                img_left_depth = self.img_left_depth_deque.popleft()
            img_left_depth = self.bridge.imgmsg_to_cv2(img_left_depth, 'passthrough')

        img_right_depth = None
        if self.args.use_depth_image:
            while self.img_right_depth_deque[0].header.stamp.to_sec() <= frame_time:
                img_right_depth = self.img_right_depth_deque.popleft()
            img_right_depth = self.bridge.imgmsg_to_cv2(img_right_depth, 'passthrough')

        img_front_depth = None
        if self.args.use_depth_image:
            while self.img_front_depth_deque[0].header.stamp.to_sec() <= frame_time:
                img_front_depth = self.img_front_depth_deque.popleft()
            img_front_depth = self.bridge.imgmsg_to_cv2(img_front_depth, 'passthrough')
