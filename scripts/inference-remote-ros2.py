...
class RosOperator(Node):
    ...
    def model_inference(self):
        ...
        inference_client = RemoteInferenceClient(
            ...
            use_compression=self.args.use_compression,
            socket_timeout=self.args.socket_timeout,
            chunk_size=self.args.network_chunk_size,
            socket_buffer_size=self.args.socket_buffer_size,
            compression_level=self.args.compression_level,
            max_retries=self.args.max_retries
        )
        
        if not inference_client.connect():
            print("Failed to connect to inference server. Exiting...")
            return
        ...
        print("Remote inference client initialized. Starting control loop...")
                ...
                if self.args.asynchronous_inference:
                    if inference_thread is None and (self.args.pos_lookahead_step == 0 or t % self.args.pos_lookahead_step == 0 or t >= max_t) and self.check_frame():
                        inference_thread = threading.Thread(target=inference_process,
                                                            args=(self.args, t, self, inference_client))
...
def get_arguments():
    ...
    # 远程服务器配置
    parser.add_argument('--server_host', type=str, default='10.21.2.65', help='Remote inference server host')
