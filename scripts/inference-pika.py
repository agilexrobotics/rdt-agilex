...
from data.hdf5_vla_dataset import decode_pose_by_incre, calc_relative_pose
...
# Add global variable for preprocessing statistics
...
def make_policy(args):
    ...
    if hasattr(args, 'stats_file_path') and args.stats_file_path:
        # Load statistics directly from specified pkl file path
        try:
            with open(args.stats_file_path, 'rb') as f:
                preprocessing_stats = pickle.load(f)
            print(f"Loaded preprocessing statistics from specified path: {args.stats_file_path}")
            print(f"  state={preprocessing_stats['state_preprocessing']}, action={preprocessing_stats['action_preprocessing']}")
        except Exception as e:
            print(f"Warning: Unable to load statistics from specified path {args.stats_file_path}: {e}")
            preprocessing_stats = None
    else:
        # Try to load statistics from checkpoint directory
        from data.hdf5_vla_dataset import HDF5VLADataset
        checkpoint_stats_path = None
        
        if args.pretrained_model_name_or_path:
            # If pretrained_model_name_or_path is a file path, get its directory
            if os.path.isfile(args.pretrained_model_name_or_path):
                checkpoint_dir = os.path.dirname(args.pretrained_model_name_or_path)
            else:
                checkpoint_dir = args.pretrained_model_name_or_path
            
            # Search for statistics files in checkpoint directory
            potential_stats_files = [
                os.path.join(checkpoint_dir, 'preprocessing_stats.pkl'),
                os.path.join(checkpoint_dir, 'preprocessing_stats_multi_pika.pkl'),
            ]
            
            for stats_file in potential_stats_files:
                if os.path.exists(stats_file):
                    checkpoint_stats_path = stats_file
                    break
        
        if checkpoint_stats_path:
            try:
                with open(checkpoint_stats_path, 'rb') as f:
                    preprocessing_stats = pickle.load(f)
                print(f"Loaded preprocessing statistics from checkpoint path: {checkpoint_stats_path}")
                print(f"  state={preprocessing_stats['state_preprocessing']}, action={preprocessing_stats['action_preprocessing']}")
            except Exception as e:
                print(f"Warning: Unable to load statistics from checkpoint path {checkpoint_stats_path}: {e}")
        
        if preprocessing_stats is None:
            print("Warning: Unable to load preprocessing statistics, will not apply preprocessing")
        else:
            print(f"Loaded preprocessing statistics: state={preprocessing_stats['state_preprocessing']}, action={preprocessing_stats['action_preprocessing']}")
...
def preprocess_state_for_inference(state_data, stats):
    """根据统计信息对状态进行预处理"""
    if stats is None or stats['state_preprocessing'] == 'none':
        return state_data
    
    if stats['state_preprocessing'] == 'standardize':
        return (state_data - stats['state_mean']) / stats['state_std']
    elif stats['state_preprocessing'] == 'normalize':
        return (state_data - stats['state_min']) / stats['state_range'] * 2 - 1
    elif stats['state_preprocessing'] == 'normalize_then_standardize':
        # 先归一化到[-1, 1]，再标准化
        normalized = (state_data - stats['state_min']) / stats['state_range'] * 2 - 1
        # 使用归一化后数据的统计信息进行标准化
        norm_mean = stats.get('state_norm_mean', np.zeros_like(stats['state_mean']))
        norm_std = stats.get('state_norm_std', np.ones_like(stats['state_std']))
        return (normalized - norm_mean) / norm_std
    else:
        return state_data


def postprocess_action_for_inference(action_data, stats):
    """根据统计信息对动作进行反预处理"""
    if stats is None or stats['action_preprocessing'] == 'none':
        return action_data
    
    if stats['action_preprocessing'] == 'standardize':
        return action_data * stats['action_std'] + stats['action_mean']
    elif stats['action_preprocessing'] == 'normalize':
        return (action_data + 1) / 2 * stats['action_range'] + stats['action_min']
    elif stats['action_preprocessing'] == 'normalize_then_standardize':
        # 先反标准化，再反归一化
        # 使用归一化后数据的统计信息进行反标准化
        norm_mean = stats.get('action_norm_mean', np.zeros_like(stats['action_mean']))
        norm_std = stats.get('action_norm_std', np.ones_like(stats['action_std']))
        denormalized = action_data * norm_std + norm_mean
        # 反归一化：从[-1, 1]恢复到原始范围
        return (denormalized + 1) / 2 * stats['action_range'] + stats['action_min']
    else:
        return action_data
...
def inference_process(args, t, ros_operator, policy):
    ...
    if camera_color is not None:
        ...
        pil_image = PImage.fromarray(np.zeros((640, 480, 3), dtype=np.uint8))
        # 重新排列图像顺序 (如果需要)
        if len(images) >= 6:
            images[3], images[4], images[5] = pil_image, images[5], images[4]
            images = images[:6]
    
    # 准备proprioception数据 (最新的关节状态)
    proprio = robot_state[0, -1, :]  # 取最新的状态
    
    # Apply state preprocessing if preprocessing stats are available
    if preprocessing_stats is not None:
        # Apply preprocessing to state
        proprio_np = proprio.reshape(1, -1)  # Shape: (1, state_dim)
        processed_state = preprocess_state_for_inference(proprio_np, preprocessing_stats)
        proprio = torch.from_numpy(processed_state.flatten()).float().cuda()
    else:
    ...
    # Apply action post-processing (reverse preprocessing) if preprocessing stats are available
    if preprocessing_stats is not None:
        action = postprocess_action_for_inference(action, preprocessing_stats)
    ...
    for i in range(len(args.arm_end_pose_names)):
        ...
        all_actions.append(decode_pose_by_incre(last_ctrl_arm_end_poses[-1][i], action[0][:, i*args.arm_end_pose_dim:(i+1)*args.arm_end_pose_dim], 0))
    all_actions = np.concatenate(all_actions, axis=-1)[np.newaxis, :]
    inference_lock.acquire()
    inference_actions = all_actions  # 1, chunk_size, action_dim
...
def model_inference(args, ros_operator):
    ...
    # 0.2315508867606016, -2.365701964007318e-06, 0.1086502962013508, -5.032693615049478e-06, 0.3140743375720001, -1.2025435587192976e-05, 1.6
    # 0.2408368057645897, 0.00287136069042693, -0.0009838999821004113, -0.024625887251749915, 0.7530362776143732, 0.006795072511917707, 1.6
...
def get_arguments():
    ...
    parser.add_argument('--stats_file_path', type=str, default=None,
                        help='Path to the preprocessing statistics pkl file')
